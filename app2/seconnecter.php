...
if (isset ($_POST["tel"]))
{
	...
	{	
		...
		if (Sql::count('Utilisateur','login=?',$_POST['login']))
		{
			error('Ce login est déjà pris.');
		}
		
		if (Sql::count('Utilisateur','mail=?',$_POST['mail']))
		{
		...
		if (!hasError())
		{
			valid("Votre inscription a été effectuée. Bienvenue sur Ebaie !");			
			Sql::insert('Utilisateur', 
						'', 
						$_POST["nom"], 
						$_POST["prenom"], 
						$_POST["jour"], 
...
else if (isset ($_POST["login"]))
{
	...
	{
		Sql::select('Utilisateur','login=? AND mdp=?',$_POST["login"], $_POST["mdp"]);
		$data = Sql::getData();
...
top('Se connecter', '<link rel="stylesheet" href="css/seconnecter.css" />');

messages();
?>

<!-- LES INDENTATIONS SONT ESSENTIELLES !!!!!!!!!!!!!!!!!!!!!!!!!!!!! -->
<!-- LES ID POUR LES ELEMENTS UNIQUES ET CLASS UNIQUEMENT POUR LES ELEMENTS MULTIPLES -->
<section id="connexionBloc">
	<h1>Se connecter</h1>
	<form method="post" action="">
		<p>
			...
			<input type="password" name="mdp" id="pass" size="25"/>
			<br/>
...
<!-- LES PIXELS SONT COMME DES ATOMES INDIVISIBLES -->

<section id="inscriptionBloc">
...
?>
		...
		<label for="login">Login</label>
		<input type="text" name="login" id="loginI"size="30"/></br>

		<label for="pass">Mot de passe</label>
		<input type="password" name="mdp" id="passI" size="30"/><br/>

		<label for="pass">Vérification mot de passe</label>
		<input type="password" name="mdp2" id="passI1" size="30"/><br/>
		</br>
